...
def appPageWizard(params) {
  if (!state.appType) {
    ...
    if (params.type) {
      ...
      def typeString = parent.getAppType()
      setAppType(typeString.toLowerCase())
    }
  }
  ...
  dynamicPage(name: 'mainPage', title: 'Lock Manager', install: true, uninstall: true, submitOnChange: true) {
    section('Create New Integration') {
      ...
      if (settings.appType) {
        ...
        app(name: 'newChild', appName: 'Lock Manager', namespace: 'ethayer', title: "Create New ${appTypeString}", multiple: true, image: "http://images.lockmanager.io/app/v1/images/new-${miniTypeString}.png")
      }
    }
    section('Locks') {
...
def lockMainPage() {
  dynamicPage(name: "lockMainPage", title: "Lock Settings", install: true, uninstall: true) {
    ...
    section('Setup', hideable: true, hidden: true) {
      input(name: 'appLabel', type: 'text', title: 'Name for app', defaultValue: 'Lock: ' + settings.lock, require: true, image: 'http://images.lockmanager.io/app/v1/images/lock.png')
      if(appLabel) {
...
def userSetupPage() {
  dynamicPage(name: 'userSetupPage', title: 'Setup User', nextPage: 'userMainPage', uninstall: true) {
...
def userMainPage() {
  ...
  dynamicPage(name: 'userMainPage', title: '', install: true, uninstall: true) {
    ...
    section('Setup', hideable: true, hidden: true) {
      input(name: 'appLabel', type: 'text', title: 'Name for app', defaultValue: 'User: ' + settings.userName, require: true, image: 'http://images.lockmanager.io/app/v1/images/user.png')
      if(appLabel) {
        app.updateLabel(appLabel)
      }
      input(name: 'userName', type: 'text', title: "Name for user", defaultValue: settings.userName, required: true, image: 'http://images.lockmanager.io/app/v1/images/user.png')
      input(name: "userSlot", type: "enum", options: parent.availableSlots(settings.userSlot), title: "Select slot", required: true, refreshAfterSelection: true )
    }
  }
}
...
def keypadMainPage() {
  dynamicPage(name: 'keypadMainPage',title: 'Keypad Settings (optional)', install: true, uninstall: true) {
    ...
    section('Setup', hideable: true, hidden: true) {
      ...
      input(name: 'appLabel', type: 'text', title: 'Name for app', defaultValue: 'Keypad: ' + settings.keypad, require: true, image: 'http://images.lockmanager.io/app/v1/images/keypad.png')
      if(appLabel) {
        app.updateLabel(appLabel)
      }
    }
